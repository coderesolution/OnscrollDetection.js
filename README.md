...
| `data-onscroll-preset`    |         |                               false                                | Automatically adjust start/bottom based on `offset` settings to eliminate the need for complex settings like `data-onscroll-start="top bottom+=200"`. Note: If you set an offset, the auto setting will set the starting point to where the original element was (before offset) to avoid a jump as it comes into view. This has no effect on `speed` as that calculates independently.                                                                                                |
